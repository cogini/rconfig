...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['taskType'])) {
        ...
        $errors['taskType'] = "Task Type cannot be emtpy";
    }
    ...
    if (!empty($_POST['taskName']) && is_string($_POST['taskName'])) {
        ...
        $errors['taskName'] = "Task Name cannot be emtpy";
    }
    ...
    if (!empty($_POST['taskDesc']) && is_string($_POST['taskDesc'])) {
        ...
        $errors['taskDesc'] = "Task Description cannot be emtpy";
    }
	...
	// test if any of the cron fields are emtpty
    ...
    if (!empty($_POST['taskName'])) {
        ...
        if ($_POST['taskType'] == 1) { //taskType is download Configurations
            ...
            $errors['taskTypeError'] = "There was a probelm selecting the Task Type"; // throw an error
            $log->Warn("Failure: There was a probelm selecting the Task Type (File: " . $_SERVER['PHP_SELF'] . ")");
        }
        ...
        */
        ...
        // Ammend all selected Nodes new TaskID with a '1' to identify an active state for this task
		
        $categories = unserialize($categories);	// unserialize $categories before next tasks becuase was serialsed for DB input

        if (!empty($categories)) {
            if (is_array($categories)) { // check if $catergories is an array because when selecting compare report, cat option is single value
...
elseif (isset($_POST['del'])) {
    ...
    // Query to retireve row for given ID
