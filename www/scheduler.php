...
<div id="mainwrap">
	...
	?>
		...
		<?php /* PHP Script for the task Sceduler Page */
		/* Includes */
		require_once("../classes/db.class.php");
		/* Instantiate DB Class */
		$db = new db();
		/* Get all Categorynames for the catSelect Selection list where NOT deleted*/
			$catQ = $db->q('SELECT `id`, `categoryName` FROM `categories` 	
							WHERE status = 1 ORDER BY `categoryName` ASC'); /* Get all Hostnames for the deviceSelect Selection list where NOT deleted*/ 
			
			$devQ = $db->q('SELECT `id`, `deviceName`, `status` FROM `nodes` 
							WHERE status = 1 ORDER BY `deviceName` ASC'); 
				...
				<fieldset id="tableFieldset">
					...
					<form name="taskForm" action="lib/crud/scheduler.crud.php" method="POST" class="myform stylizedForm stylized" style="width:100%;">
						...
						<?php	// echo error message if is sent back in GET from CRUD
							if(isset($errors['taskType'])){echo "<span class=\"error\">".$errors['taskType']."</span>";} ?>
						</label>
						<?php //http://stackoverflow.com/questions/3988938/onclick-in-select-not-working-in-ie ?>
						<select name="taskType" id ="taskType" size="2" 
						onchange="switch(this.value){
							case '1': hideReportElements(); 
							break; 
							case '2': displayReportElements(); 
							break; 
						}">
						...
						<div name="reportTypeSlctDiv" id ="reportTypeSlctDiv">
							...
							<select name="reportTypeSlct[]" id="reportTypeSlct">
								...
								<?php echo reportsOptions(); // output full report name list options?>
							</select>
						</div>
						<div class="spacer">
						</div>
						...
						<?php 
							if(isset($errors['taskName'])){echo "<span class=\"error\">".$errors['taskName']."</span><br/>";} ?>
						</label>
						...
						<?php	// echo error message if is sent back in GET from CRUD
							if(isset($errors['taskDesc'])){echo "<span class=\"error\">".$errors['taskDesc']."</span>";} ?>
						</label>
						...
						<div id="chooseCatDiv">
							...
							if(isset($errors['catId'])){echo "<span class=\"error\">".$errors['catId']."</span>";} ?>
						</label>
						...
						<div id="catCommandDiv" style="display:none;">
							...
							if(isset($errors['catCommand'])){echo "<span class=\"error\">".$errors['catCommand']."</span>";} ?>
						</label>
						...
						<div id="deviceSelectRadioDiv">
							...
							<?php	// echo error message if is sent back in GET from CRUD
								if(isset($errors['selectRadio'])){echo "<span class=\"error\">".$errors['selectRadio']."</span>";} 
								if(isset($errors['deviceSelectRadio'])){echo "<span class=\"error\">".$errors['deviceSelectRadio']."</span>";} ?>
						</label>
							...
							<select name="deviceSelect[]" id ="deviceSelect" size="10" multiple disabled>
								<?php
										while($row = mysql_fetch_assoc($devQ)){  
											echo "<option value=".$row['id'].">
								".$row['deviceName']."</option>
								"; } ?>
							</select>
						</div>
						<div class="spacer">
						...
						<div id="catSelectRadioDiv">
							...
							<?php 
								if(isset($errors['catSelectRadio'])){echo "<span class=\"error\">".$errors['catSelectRadio']."</span>";} ?>
						</label>
							...
							<select name="catSelect[]" id ="catSelect" size="10" multiple disabled>
								<?php
										while($row = mysql_fetch_assoc($catQ)){  
						...
						<div id="contabDiv">
							...
							<?php	// echo error message if is sent back in GET from CRUD
								if(isset($errors['cron'])){echo "<span class=\"error\">".$errors['cron']."</span><br/>";}
							?>
							<div class="spacer">
							</div>
