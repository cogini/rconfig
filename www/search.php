				...
				<div class="mainformDiv">
					<form id="searchForm" method="post" enctype="multipart/form-data" class="myform stylizedForm stylized">
						...
						<?php	// echo error message if is sent back in GET from CRUD
							if(isset($errors['searchTerm'])){echo "<br /><span class='\"error\"'>".$errors['searchTerm']."</span>";} ?> 
						...
						<?php if(isset($errors['catId'])){echo "<br /><span class='\"error\"'>".$errors['catId']."</span>";}?>
						<div class="spacer"></div>
						...
						<fieldset id="tableFieldset">
							...
							<!-- extra add params to search - removed
							<br/><a href="javascript:optionalGrep()">Add Search Parameter </a>
							<div id="testDiv">
							</div>
							<!-- Template Input filed for dynamic input addition 
							<div id="newInputField" style="display:none">
								<select class="optional" id="containsSelect" name="containsSelect[]">
									<option value="">Search Contains -</option>
									<option value="-v">Does Not Contain -</option>
								</select>
								<input class="optional" type="text" id="grepField" name="grepField[]" value="" size="50" placeholder="string here..">
							</div>
							<!-- Template Input filed for dynamic input addition 
							-->
						</fieldset>
