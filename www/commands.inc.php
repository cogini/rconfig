...
$rs              = $db->q('SELECT COUNT(*) AS total FROM configcommands WHERE status = 1');
...
<table id="commandsTbl" class="tableSimple">
	...
	foreach ($result['rows'] as $rows):
		...
		$catQ    = $db->q('SELECT cct.nodeCatId, cat.categoryName
	...
	// get Query results and push to catName array then impode to string
		...
		echo '<td align="center"><input type="checkbox" id="' . $cmdId . '"></td>';
